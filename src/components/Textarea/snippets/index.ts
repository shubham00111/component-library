export const neobrutalismJsx = `const NeobrutalismTextarea = ({ value, onChange, placeholder = "", disabled = false, label, error, helperText, rows = 4, id }) => {
  ...
  return (
    <div className="flex flex-col gap-1 font-sans">
      {label && (
        <label htmlFor={inputId} className="text-[0.8125rem] font-extrabold text-black">{label}</label>
      )}
      <textarea
        id={inputId}
        value={inputValue}
        ...
        className={[
          "py-2 px-3 text-[0.9375rem] font-sans text-black bg-white border-[3px] border-black rounded-[2px] shadow-[4px_4px_0_#000] outline-none w-full resize-y",
...
export const shadcnJsx = `const ShadcnTextarea = ({ value, onChange, placeholder = "", disabled = false, label, error, helperText, rows = 4, id }) => {
  const [inputValue, setInputValue] = useState(value ?? "");
  const inputId = id ?? (label ? label.toLowerCase().replace(/\\s+/g, "-") : undefined);
  ...
  return (
    <div className="flex flex-col gap-1.5 font-sans">
      {label && (
        <label htmlFor={inputId} className="text-sm font-medium text-zinc-900">{label}</label>
      )}
      <textarea
        id={inputId}
        value={inputValue}
        ...
        onChange={(e) => { setInputValue(e.target.value); onChange?.(e); }}
        className={[
          "py-2 px-3 text-[0.9375rem] font-sans text-zinc-900 bg-white border border-zinc-200 rounded-[6px] outline-none w-full resize-y transition-[border-color,box-shadow] duration-150",
...
export const flowbiteJsx = `const FlowbiteTextarea = ({ value, onChange, placeholder = "", disabled = false, label, error, helperText, rows = 4, id }) => {
  const [inputValue, setInputValue] = useState(value ?? "");
  const inputId = id ?? (label ? label.toLowerCase().replace(/\\s+/g, "-") : undefined);
  ...
  return (
    <div className="flex flex-col gap-1.5 font-sans">
      {label && (
        <label htmlFor={inputId} className="text-sm font-medium text-gray-900">{label}</label>
      )}
      <textarea
        id={inputId}
        value={inputValue}
        ...
        onChange={(e) => { setInputValue(e.target.value); onChange?.(e); }}
        className={[
          "py-2 px-3 text-[0.9375rem] font-sans text-gray-900 bg-white border border-gray-200 rounded-lg shadow-sm outline-none w-full resize-y transition-all duration-200",
