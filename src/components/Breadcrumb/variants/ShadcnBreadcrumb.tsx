...
const Item = ({ label, href, className = "" }: BreadcrumbItemProps) => {
  return href ? (
    <a
      href={href}
      className={["text-sm font-medium text-zinc-500 hover:text-zinc-900 transition-colors duration-150", className].filter(Boolean).join(" ")}
    >
      {label}
    </a>
  ) : (
    <span className={["text-sm font-medium text-zinc-900", className].filter(Boolean).join(" ")} aria-current="page">
      {label}
    </span>
  );
};
        ...
        {items.map((child, index) => (
          <li key={index} className="flex items-center">
            ...
            {index < items.length - 1 && (
              <span className="mx-2 text-sm text-zinc-400 select-none">/</span>
            )}
          </li>
        ))}
      </ol>
